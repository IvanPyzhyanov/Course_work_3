...
class FavoriteMovieView(Resource):
    ...
    def post(self, movie_id: int):
        """Add favorite movie"""
        user_id = auth_check().get("id")
        return FavoriteMovieService(db.session).create(movie_id=movie_id, user_id=user_id)

    def delete(self, movie_id: int):
        """Delete favorite movie"""
        user_id = auth_check().get("id")
        return FavoriteMovieService(db.session).delete(movie_id=movie_id, user_id=user_id)